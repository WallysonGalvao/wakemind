  ...
  return (
    ...
    >
      ...
      {isPlatinum && isUnlocked ? (
        <View className="absolute left-0 top-0 h-0.5 w-full bg-gradient-to-r from-transparent via-cyan-500 to-transparent opacity-50" />
      ) : null}
      ...
      <View className={`flex h-28 items-center justify-center p-4 ${colors.iconBg}`}>
        <View
          className={`flex h-12 w-12 items-center justify-center rounded-full ${isPlatinum && isUnlocked ? 'border border-cyan-500/20 bg-gradient-to-tr from-white to-cyan-500/20' : colors.iconBg}`}
        >
          <MaterialSymbol
            name={def.icon}
            size={32}
