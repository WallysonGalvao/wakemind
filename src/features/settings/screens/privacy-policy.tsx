...
function SectionTitle({ icon, title }: { icon: string; title: string }) {
  return (
    <View className="mb-4 flex-row items-center gap-3">
      <MaterialSymbol name={icon} size={20} color={COLORS.brandPrimary} style={{ opacity: 0.6 }} />
      <Text className="text-xl font-bold tracking-tight text-primary-500">{title}</Text>
    </View>
  );
}
  ...
  return (
    <View className="mb-4 flex-row gap-4">
      <View className="mt-1.5">
        <View className="h-3.5 w-3.5 items-center justify-center rounded-sm border border-primary-500 bg-background-dark">
          <View className="h-1 w-1 rounded-[1px] bg-primary-500" />
        </View>
      </View>
      <View className="flex-1">
        <Text className="mb-1 text-sm font-medium text-white">{title}</Text>
        <Text className="text-xs leading-relaxed text-gray-400">{description}</Text>
      </View>
    </View>
  );
}
...
function NotCollectedItem({ text }: { text: string }) {
  return (
    <View className="mb-3 flex-row items-center gap-3">
      <MaterialSymbol name="block" size={18} color="#f87171" style={{ opacity: 0.8 }} />
      <Text className="text-sm font-medium text-gray-200">{text}</Text>
    </View>
  );
}
...
export default function PrivacyPolicyScreen() {
  ...
  return (
    <View className="flex-1 bg-background-dark">
      ...
      >
        {/* Metadata Header */}
        <View className="mb-10 items-center opacity-70">
          <View className="mb-2 h-8 w-[1px] bg-primary-500/50" />
          <Text className="text-xs font-medium uppercase tracking-widest text-primary-500/80">
            {t('privacyPolicy.lastUpdatedLabel')}
          </Text>
          <Text className="font-mono mt-0.5 text-[11px] text-white/60">
        ...
        <View className="mb-12">
          ...
          <Text className="text-[15px] font-light leading-loose text-gray-200">
        ...
        <View className="relative mb-12">
          ...
          <View className="absolute bottom-4 left-[7px] top-8 w-[1px] bg-white/10" />
        ...
        <View className="relative mb-12 overflow-hidden rounded-lg">
          {/* Background */}
          <View className="absolute inset-0 bg-primary-500/5" />
          {/* Top line */}
          <View className="absolute left-0 right-0 top-0 h-[1px] bg-primary-500/40" />
          {/* Bottom line */}
          <View className="absolute bottom-0 left-0 right-0 h-[1px] bg-primary-500/40" />

          <View className="relative border-l-2 border-primary-500 p-6">
            <View className="mb-5 flex-row items-center justify-between">
              <Text className="text-xl font-bold tracking-tight text-primary-500">
            ...
            <View className="mt-6 border-t border-white/5 pt-4">
              <Text className="font-mono text-xs italic text-primary-500/70">
        ...
        <View className="mb-12">
          ...
          <Text className="mb-4 text-[15px] font-light leading-loose text-gray-200">
            {t('privacyPolicy.security.content')}
          </Text>
          <View className="flex-row items-center gap-2 self-start rounded border border-white/10 bg-white/5 px-3 py-1.5">
            <MaterialSymbol name="check_circle" size={14} color="#4ade80" />
        ...
        {/* Footer */}
        <View className="items-center border-t border-white/5 pt-8">
          <MaterialSymbol
            name="fingerprint"
            size={32}
            color={COLORS.white}
            style={{ opacity: 0.2 }}
          />
          <Text className="mb-3 mt-4 text-center text-xs font-light text-gray-500">
            {t('privacyPolicy.footer.description')}
          </Text>
          <Pressable onPress={handleContactPress} accessibilityRole="link">
            <Text className="text-sm font-medium text-primary-500 underline">
