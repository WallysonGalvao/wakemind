...
export default function WakeConsistencyInfoScreen() {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle: t('dashboard.wakeConsistency.infoModal.title'),
      headerLeft: () => (
      ...
      <ScrollView contentContainerClassName="pb-16" showsVerticalScrollIndicator={false}>
        ...
        <View className="mb-8 px-6 pt-6">
          <Text className="mb-2 text-[10px] font-bold uppercase tracking-[0.2em] text-brand-primary">
            {t('dashboard.wakeConsistency.infoModal.performanceMetrics')}
          </Text>
          <Text className="mb-2 text-3xl font-bold leading-tight text-slate-900 dark:text-white">
            {t('dashboard.wakeConsistency.infoModal.title')}
          </Text>
          <Text className="text-sm leading-relaxed text-slate-500 dark:text-slate-400">
            {t('dashboard.wakeConsistency.infoModal.description')}
          </Text>
        </View>
        ...
        <View className="relative px-6">
          ...
          <View className="relative z-10 flex flex-col">
            <MetricCard
              index={1}
              icon="target"
              label={t('dashboard.wakeConsistency.infoModal.targetTimeLabel')}
              title={t('dashboard.wakeConsistency.infoModal.targetTime.title')}
              description={t('dashboard.wakeConsistency.infoModal.targetTime.description')}
            />

            <MetricCard
              index={2}
              icon="avg_time"
              label={t('dashboard.wakeConsistency.infoModal.averageTimeLabel')}
              title={t('dashboard.wakeConsistency.infoModal.averageTime.title')}
              description={t('dashboard.wakeConsistency.infoModal.averageTime.description')}
            />

            <MetricCard
              index={3}
              icon="analytics"
              label={t('dashboard.wakeConsistency.infoModal.varianceLabel')}
              title={t('dashboard.wakeConsistency.infoModal.variance.title')}
              description={t('dashboard.wakeConsistency.infoModal.variance.description')}
              isLast
            />
          </View>
        </View>
        ...
        <View className="mx-6 mt-10 overflow-hidden rounded-2xl bg-slate-900 p-5 dark:bg-slate-800">
          <View className="absolute right-0 top-0 p-4 opacity-10">
            <MaterialSymbol name="schedule" size={64} className="text-white" />
          </View>
          <View className="relative z-10">
            <Text className="mb-2 text-sm font-bold uppercase tracking-widest text-brand-primary">
              {t('dashboard.wakeConsistency.infoModal.consistencyGoal')}
            </Text>
            <Text className="mb-4 text-xs leading-relaxed text-slate-300">
              {t('dashboard.wakeConsistency.infoModal.consistencyGoalDescription')}
            </Text>
            <View className="border-t border-white/10 pt-4">
              <View className="flex-row items-center justify-between">
                <Text className="text-[10px] font-bold uppercase text-slate-400">
                  {t('dashboard.wakeConsistency.infoModal.optimalVariance')}
                </Text>
                <Text className="text-xl font-bold tabular-nums text-white">&lt;5m</Text>
              </View>
            </View>
          </View>
        </View>
