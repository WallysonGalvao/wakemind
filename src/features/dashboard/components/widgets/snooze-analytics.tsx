...
export function SnoozeAnalytics({
  ...
  const getTrendColor = () => {
    if (trendDirection === 'down') return COLORS.success;
    if (trendDirection === 'up') return COLORS.error;
  ...
  const getPerformanceStatus = () => {
    if (firstTouchRate >= 80)
      return { text: t('dashboard.snooze.excellent'), color: COLORS.success };
    if (firstTouchRate >= 60)
      return { text: t('dashboard.snooze.good'), color: COLORS.brandPrimary };
    if (firstTouchRate >= 40) return { text: t('dashboard.snooze.fair'), color: COLORS.warning };
    return { text: t('dashboard.snooze.needsWork'), color: COLORS.error };
  };
      ...
      <View className="flex-row gap-4">
        ...
        <View className="flex-1 rounded-lg bg-slate-50 p-3 dark:bg-slate-900">
          ...
          <View className="mt-1 flex-row items-end gap-1">
            ...
            />
            <Text className="mb-0.5 text-lg font-semibold text-gray-500 dark:text-gray-400">%</Text>
          </View>
        </View>
      </View>
